...
import { createCurug, updateCurug, uploadCurugImage } from '@/app/admin/curugs/actions'
...
const curugFormSchema = z.object({
  ...
  google_maps_url: z.union([z.string().url('URL tidak valid'), z.literal('')]).optional(),
  difficulty: z.enum(['mudah', 'menengah', 'sulit', '']).optional(),
  trek_duration_minutes: z.union([z.coerce.number().int().nonnegative(), z.literal('')]).optional(),
  distance_from_city_km: z.union([z.coerce.number().nonnegative(), z.literal('')]).optional(),
...
type CurugFormValues = z.infer<typeof curugFormSchema>

export function CurugForm({ curug }: { curug: Curug | null }) {
  const router = useRouter()
  const isNew = !curug

  const [facilities, setFacilities] = useState<string[]>(curug?.facilities ?? [])
  const [tags, setTags] = useState<string[]>(curug?.tags ?? [])
  const [coverImageUrl, setCoverImageUrl] = useState<string | null>(curug?.cover_image_url ?? null)
  const [gallery, setGallery] = useState<string[]>(curug?.gallery ?? [])
  const [isPublished, setIsPublished] = useState<boolean>(curug?.is_published ?? true)
  ...
  function onSubmit(values: CurugFormValues) {
    ...
    startTransition(async () => {
      const result = isNew ? await createCurug(formData) : await updateCurug(curug!.id, formData)
      if (!result.success) {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="mx-auto max-w-3xl space-y-6" noValidate>
      <Section title="Informasi Dasar">
        <Field label="Nama curug" error={errors.name?.message}>
          <input {...register('name')} className={inputClass} placeholder="Curug Cipendok" />
        </Field>
        <Field label="Nama alternatif" error={errors.alt_name?.message}>
          <input {...register('alt_name')} className={inputClass} placeholder="Opsional" />
        </Field>
        ...
        <Field label="Deskripsi singkat" error={errors.short_description?.message}>
          <textarea {...register('short_description')} rows={2} className={inputClass} />
        </Field>
      ...
      <Section title="Lokasi">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <Field label="Latitude" error={errors.latitude?.message as string}>
            <input {...register('latitude')} className={inputClass} placeholder="-7.3xxx" />
          </Field>
          <Field label="Longitude" error={errors.longitude?.message as string}>
            <input {...register('longitude')} className={inputClass} placeholder="109.2xxx" />
          </Field>
        </div>
        <Field label="Google Maps URL" error={errors.google_maps_url?.message}>
          <input {...register('google_maps_url')} className={inputClass} placeholder="https://maps.google.com/..." />
        </Field>
      </Section>
      ...
      <Section title="Detail Trek">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <Field label="Durasi trek (menit)" error={errors.trek_duration_minutes?.message as string}>
            <input {...register('trek_duration_minutes')} className={inputClass} />
          </Field>
          <Field label="Jarak dari kota (km)" error={errors.distance_from_city_km?.message as string}>
            <input {...register('distance_from_city_km')} className={inputClass} />
          </Field>
          <Field label="Harga tiket masuk (Rp)" error={errors.ticket_price?.message}>
            <input {...register('ticket_price')} className={inputClass} />
          </Field>
          <Field label="Harga parkir (Rp)" error={errors.parking_price?.message}>
            <input {...register('parking_price')} className={inputClass} />
          </Field>
        </div>
        <Field label="Waktu terbaik berkunjung" error={errors.best_time_to_visit?.message}>
          <input {...register('best_time_to_visit')} className={inputClass} placeholder="Musim kemarau, pagi hari" />
        </Field>
        <Field label="Catatan akses jalan" error={errors.access_notes?.message}>
          <textarea {...register('access_notes')} rows={2} className={inputClass} />
        </Field>
      </Section>

      <Section title="Fasilitas & Tag">
        <CheckboxGroup label="Fasilitas" options={FACILITY_OPTIONS} value={facilities} onChange={setFacilities} />
        <CheckboxGroup label="Tag" options={TAG_OPTIONS} value={tags} onChange={setTags} />
      </Section>

      <Section title="Foto">
        <ImageUploader label="Cover Image" value={coverImageUrl} onChange={setCoverImageUrl} uploadAction={uploadCurugImage} />
        <GalleryUploader value={gallery} onChange={setGallery} uploadAction={uploadCurugImage} />
      </Section>
